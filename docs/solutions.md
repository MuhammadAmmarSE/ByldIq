...
| Full CLAUDE.md Part 8 nav (Work, Process, Technology, About, Contact) | Only `Knowledge`, `BuildPath`, and the new `Solutions` dropdown are real — the rest have no pages yet.                                                                                                                                                                                      |
| A 404 status code for an unknown `/solutions/[slug]`                  | Renders the correct not-found UI but returns HTTP 200 — a pre-existing Next.js 15 behavior for statically-generated `[slug]` routes calling `notFound()`, reproduced identically on `/case-studies/[slug]`. Framework-level, not a Solutions-specific regression; out of scope to fix here. |
