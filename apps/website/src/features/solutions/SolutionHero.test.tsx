import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

const { mockTrack } = vi.hoisted(() => ({ mockTrack: vi.fn() }));

vi.mock("@/providers/AnalyticsProvider", () => ({
  useAnalytics: () => ({ track: mockTrack }),
}));

import {
  AiCompanionStoreProvider,
  useAiCompanionStore,
} from "@/providers/AiCompanionStoreProvider";
import { StoreProvider } from "@/providers/StoreProvider";

import { SOLUTIONS } from "./data/solutions";
import { SolutionHero } from "./SolutionHero";

function requireStartupSolution() {
  const found = SOLUTIONS.find((candidate) => candidate.slug === "startup");
  if (!found) throw new Error("Missing startup solution fixture");
  return found;
}

const solution = requireStartupSolution();

function renderHero() {
  return render(
    <StoreProvider>
      <AiCompanionStoreProvider>
        <SolutionHero solution={solution} />
      </AiCompanionStoreProvider>
    </StoreProvider>,
  );
}

describe("SolutionHero", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  afterEach(() => {
    mockTrack.mockClear();
  });

  it("renders the breadcrumb, headline, and who-it's-for copy", () => {
    renderHero();

    expect(screen.getByRole("navigation", { name: "Breadcrumb" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: solution.heroHeadline })).toBeInTheDocument();
    expect(screen.getByText(solution.who)).toBeInTheDocument();
  });

  it("tracks solution_viewed once on mount", () => {
    renderHero();
    expect(mockTrack).toHaveBeenCalledWith("solution_viewed", { slug: solution.slug });
  });

  it("links the primary CTA to BuildPath and tracks the click", async () => {
    const user = userEvent.setup();
    renderHero();

    const cta = screen.getByRole("link", { name: solution.primaryCtaLabel });
    expect(cta).toHaveAttribute("href", "/buildpath");

    await user.click(cta);
    expect(mockTrack).toHaveBeenCalledWith("solution_cta_selected", {
      slug: solution.slug,
      cta: "hero-primary",
    });
  });

  it("opens the AI companion from Talk to Byld", async () => {
    const user = userEvent.setup();

    function Harness() {
      const isOpen = useAiCompanionStore((state) => state.isOpen);
      return (
        <>
          <SolutionHero solution={solution} />
          <p data-testid="ai-open-state">{isOpen ? "open" : "closed"}</p>
        </>
      );
    }

    render(
      <StoreProvider>
        <AiCompanionStoreProvider>
          <Harness />
        </AiCompanionStoreProvider>
      </StoreProvider>,
    );

    await user.click(screen.getByRole("button", { name: /talk to byld/i }));
    expect(screen.getByTestId("ai-open-state")).toHaveTextContent("open");
    expect(mockTrack).toHaveBeenCalledWith("solution_cta_selected", {
      slug: solution.slug,
      cta: "ai",
    });
  });
});
